...
class _AddFriendTabState extends State<AddFriendTab> with SingleTickerProviderStateMixin {
  ...
  Future<void> _handlePick(FriendSearchPick pick) async {
    if (pick is FriendSearchPickUser) {
      try {
        await context.read<FriendsRepository>().addFriendById(pick.profile.userId);
        if (!mounted) return;
        final msg = context.trRead('friendsAdded');
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
        await _loadExistingFriendIds();
      } catch (e) {
        if (!mounted) return;
    ...
    if (pick is FriendSearchPickCharacter) {
      await _openCharacterChat(pick.record);
    }
  }
          ...
          : TabBarView(
              ...
              children: [
                AddFriendPeoplePanel(
                  ...
                  onPickUser: (profile) => _handlePick(FriendSearchPickUser(profile)),
                ),
                AddFriendCharacterPanel(
                  myUserId: myId,
                  characterRecordRepository: charRepo,
                  onPickCharacter: (record) => _handlePick(FriendSearchPickCharacter(record)),
...
class AddFriendPeoplePanel extends StatefulWidget {
  ...
  final void Function(UserProfileSearchResult profile) onPickUser;
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (ctx, i) {
                              ...
                              return ListTile(
                                ...
                                trailing: Icon(Icons.person_add_alt_1_outlined, color: Theme.of(context).colorScheme.primary),
                                onTap: () => widget.onPickUser(r),
                              );
                            },
                          ),
          ),
